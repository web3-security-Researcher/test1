...
import "@contracts/Non-Fungible-Receipts/veNFTS/Aerodrome/veNFTAerodrome.sol";
import "@openzeppelin/contracts/token/ERC20/ERC20.sol";

contract veNFTAerodrome is ReentrancyGuard, ERC721Enumerable {
    event createdVault(
        uint receiptID,
        address newVault,
        address underlying,
    ...
    struct receiptInstance {
        uint receiptID; // ID of the receipt
        uint attachedNFT; // ID of the NFT attached to the receipt
        uint lockedAmount; // Amount of the NFT locked
        uint lockedDate; // Unlock date of the NFT locked
        uint decimals; // Decimals of the underlying token
    ...
    mapping(uint => address) public s_ReceiptID_to_Vault; // receiptID => vault
    mapping(address => bool) internal isVaultValid; // is the vault created by this contract

    mapping(address => uint) public balanceOfManagement; // user address => amount of NFTs managing
    mapping(address => mapping(uint => uint)) internal ownedTokens; // user address => index = nftID
    mapping(address => mapping(uint => uint)) internal indexPosition; // address  => nft = index position

    uint private s_ReceiptID;
    address public _underlying; // Address of the underlying token

    constructor(
        address _nftAddress,
        address underlying
    ) ERC721("NFR veAERO", "ReceiptveNFT") {
    ...
    /**
     * @notice Deposit NFTs to create a receipt
     * @param nftsID Array of NFTs ID to deposit --> in this case would be of veAERO tokens
     */
